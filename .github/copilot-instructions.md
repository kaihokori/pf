# Trackerio – Copilot Instructions
...
## Core Context
- Frontend iOS client for Trackerio; server APIs live elsewhere. Keep SwiftUI-first, target iOS 16+ (current scaffolding in `Trackerio/Pump_FitnessApp.swift`).
...
- App entry point `Trackerio/Pump_FitnessApp.swift` launches `RootView`. Expand `RootView.swift` into a `NavigationStack`-based shell with tab scaffolding for Nutrition, Workouts, Coaching, and Analytics.
...
## Workflows & Tooling
- Xcode is the build tool; open `Trackerio.xcodeproj` and run on an iOS 26+ simulator. Synchronize Firebase config (`GoogleService-Info.plist`) and USDA keys via Xcode build settings or xcconfigs.
